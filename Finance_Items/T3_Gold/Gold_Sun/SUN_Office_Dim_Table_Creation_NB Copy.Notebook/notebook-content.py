...
gold_office_table = df.select(
                        "region_name",
                        "bu_code",
                        
                    ).distinct()
display(gold_office_table.head(10))
...
df_dim_dupes = (
    gold_office_table.groupBy("bu_code", "region_name")
          .count()
...
gold_table_name = f"gold_sun_dim_office"
...
gold_office_table.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(f"Gold_Finance_LH.dbo.{gold_table_name}")
